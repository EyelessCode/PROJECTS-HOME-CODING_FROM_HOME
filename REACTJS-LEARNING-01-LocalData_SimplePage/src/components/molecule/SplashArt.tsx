...
const SplashArt:React.FC<Items> = ({description,title,url}) => {
    return (
        <div className="relative min-h-[100dvh] flex">
            <section className={`absolute bg-center w-full h-183 bg-cover flex opacity-80 inset-0`} style={{backgroundImage:`url('${url}')`}}>
                {/* <img src={url} alt="" /> */}
            </section>
            <article className="flex justify-center items-center flex-col text-center text-black z-40">
                <h2 className="font-[Rampart_One,sans_serif] font-extrabold tracking-[0.2em] text-5xl">{title}</h2>
                <p className="font-bold mt-6 font-[Arial]">{description}</p>
            </article>
